"""End-to-end maintenance bundle flow through apply-ready and semantic API."""
...
def test_apply_ready_review_bundle_supports_maintenance_guidance_candidates() -> None:
    """Prepared maintenance bundle should apply reviewed maintenance/diagnostic packs into the semantic route."""
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for entry in payload["candidate_entries"]:
                ...
                entry["curation"]["applicability"] = {"brand": "Guoxiang", "model_family": "KMS"}
                if entry["knowledge_object_type"] == "maintenance_procedure":
                    entry["curation"]["title"] = "Reviewed Clean Heat Exchange Fins"
                    entry["curation"]["summary"] = "Reviewed maintenance guidance for cleaning fins."
                    entry["curation"]["structured_payload"]["task_type"] = "cleaning"
                    entry["curation"]["structured_payload"]["maintenance_task"] = "cleaning"
                else:
                    entry["curation"]["title"] = "Reviewed Check Water Flow and Vent Air"
                    entry["curation"]["summary"] = "Reviewed diagnostic guidance for cleaning conditions."
                    entry["curation"]["structured_payload"]["task_type"] = "cleaning"
