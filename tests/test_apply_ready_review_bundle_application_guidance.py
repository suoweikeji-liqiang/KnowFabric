"""End-to-end drive application guidance bundle flow through apply-ready and API."""
...
DRIVE_APPLICATION_FIXTURE = REPO_ROOT / "tests/fixtures/manual_validation/drive_application_guidance.json"
...
def test_apply_ready_review_bundle_supports_application_guidance_candidates() -> None:
    """Prepared application bundle should apply reviewed guidance into the new semantic route."""

    client, session_factory = _build_client()
    original_session_locals = {}
    try:
        _seed_ontology(session_factory)
        _seed_fixture_chunks(session_factory, DRIVE_APPLICATION_FIXTURE)
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            bootstrapped_pack = Path(manifest["paths"]["bootstrapped_review_pack_dir"]) / (
                "drive__doc_danfoss_fc111_manual__variable_frequency_drive.json"
            )
            ...
            for entry in payload["candidate_entries"]:
                ...
                entry["curation"]["title"] = "Reviewed Pump and Fan Application Control"
                entry["curation"]["summary"] = "Reviewed application guidance for pump and fan control."
                entry["curation"]["applicability"] = {"brand": "Danfoss", "model_family": "FC111"}
        ...
        response = client.get(
            ...
            "?application_type=pump_fan&brand=Danfoss&min_trust_level=L2"
        )
...
if __name__ == "__main__":
    test_apply_ready_review_bundle_supports_application_guidance_candidates()
    print("Apply-ready application guidance bundle checks passed")
