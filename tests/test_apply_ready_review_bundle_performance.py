"""End-to-end performance-spec bundle flow through apply-ready and semantic API."""
...
def test_apply_ready_review_bundle_supports_performance_spec_candidates() -> None:
    """Prepared performance bundle should apply reviewed specs into the parameter/performance route."""
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            bootstrapped_pack = Path(manifest["paths"]["bootstrapped_review_pack_dir"]) / (
                "hvac__doc_tica_screw_chiller_manual__screw_chiller.json"
            )
            ...
            for entry in payload["candidate_entries"]:
                ...
                entry["curation"]["applicability"] = {
                    "brand": "TICA",
                    "model_family": "Water-Cooled Screw Chiller",
                }
                if entry["structured_payload_candidate"]["parameter_category"] == "capacity":
                    entry["curation"]["title"] = "Reviewed Rated Cooling Capacity"
                    entry["curation"]["summary"] = "Reviewed rated cooling capacity from the technical manual."
                else:
                    entry["curation"]["title"] = "Reviewed Nominal COP"
                    entry["curation"]["summary"] = "Reviewed nominal COP from the technical manual."
        ...
        assert {item["knowledge_object_type"] for item in payload["data"]["items"]} == {"performance_spec"}
    finally:
...
if __name__ == "__main__":
    test_apply_ready_review_bundle_supports_performance_spec_candidates()
    print("Apply-ready performance bundle checks passed")
