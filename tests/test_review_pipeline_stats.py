"""Tests for review pipeline stats summarization."""
...
def test_summarize_review_pipeline_stats_combines_candidates_packs_and_apply(monkeypatch) -> None:
    """Stats report should combine hit rate, review decisions, and apply outcomes."""
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        aux_payload = json.loads(aux_pack.read_text(encoding="utf-8"))
        for entry in aux_payload["candidate_entries"]:
            if entry["canonical_key_candidate"] == "E22":
                ...
                entry["curation"]["summary"] = "Reviewed from stats flow."
                entry["curation"]["applicability"] = {"brand": "AUX", "model_family": "X Module Unit"}
            else:
                entry["review_decision"] = "rejected"
        aux_pack.write_text(json.dumps(aux_payload, ensure_ascii=False, indent=2), encoding="utf-8")

        readiness_report = check_review_pack_directory(pack_dir)
        apply_report = apply_review_packs_in_directory(pack_dir)
    ...
    assert stats["overall"] == {
        ...
        "review_packs_total": 2,
        "review_packs_ready_to_apply": 1,
        "review_packs_pending": 1,
        "review_packs_rejected_only": 0,
    ...
    assert stats["documents"] == [
        {
            ...
            "candidate_entries": 3,
            "candidate_hit_rate": 1.0,
            "review_decisions": {"accepted": 1, "rejected": 2, "pending": 0},
            "packs_total": 1,
            "packs_ready_to_apply": 1,
            "readiness_status_counts": {
                "ready": 1,
                "blocked_pending": 0,
                "blocked_no_accepted": 0,
        ...
        },
        {
            ...
            "candidate_entries": 2,
            "candidate_hit_rate": 1.0,
            "review_decisions": {"accepted": 0, "rejected": 0, "pending": 2},
            "packs_total": 1,
    ...
    assert stats["review_packs"] == [
        ...
        },
        {
            ...
            "readiness_status": "blocked_pending",
            "readiness_blocker": "pending_review_decisions",
            "apply_status": "skipped_pending",
        },
    ]
...
if __name__ == "__main__":
    ...
    test_summarize_review_pipeline_stats_combines_candidates_packs_and_apply(monkeypatch)
    monkeypatch.undo()
    print("Review pipeline stats checks passed")
