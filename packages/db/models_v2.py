...
class KnowledgeObjectV2(Base):
    ...
    authority_summary_json = Column(JSON, nullable=True)
    consensus_state = Column(String(32), nullable=True, default="single_source")
    conflict_summary = Column(Text, nullable=True)
    highest_authority_level = Column(String(32), nullable=True)
    deviation_justification_json = Column(JSON, nullable=True)
...
class KnowledgeObjectEvidenceV2(Base):
    ...
    authority_role = Column(String(64), nullable=True)
    evidence_citation = Column(String(256), nullable=True)
    confidence_score = Column(Float)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
