...
async def get_fault_knowledge(
    ...
    min_trust_level: str = Query("L1", pattern="^L[1-4]$", description="Minimum trust level (L1=lowest threshold, includes all; L4=highest, strictest)"),
    min_authority_level: Optional[str] = Query(None, description="Minimum authority level filter"),
    consensus_filter: Optional[str] = Query(None, description="Filter by consensus state"),
    include_restricted_evidence: bool = Query(False, description="Include verbatim evidence from restricted documents"),
    limit: int = Query(100, ge=1, le=100, description="Result limit"),
